...
export default {
    ...
    async execute(interaction, config, client) {
        const wrappedExecute = withErrorHandling(
            async () => {
                ...
                switch (subcommand) {
                    ...
                    case 'dashboard':
                        return await verificationDashboard.execute(
                            interaction,
                            config,
                            client
                        );
...
async function handleSetup(interaction, guild, client) {
    ...
    const missingChannelPerms =
        requiredChannelPermissions.filter(
            perm => !verificationChannel.permissionsFor(botMember).has(perm)
        );
    ...
    if (verifiedRole.position >= botRole.position) {
        throw createError(
            ...
            {
                rolePosition: verifiedRole.position,
    ...
    const hasAutoVerifyEnabled =
        Boolean(guildConfig.verification?.autoVerify?.enabled);

    const hasAutoRoleConfigured =
        Boolean(guildConfig.autoRole) ||
        (
            Array.isArray(welcomeConfig.roleIds) &&
    ...
    /*
     * ============================================================
     * RIXZY LEAKS VERIFICATION PANEL
     * ============================================================
     */
    ...
    const defaultMessage = [
        '🇪🇸 **VERIFICACIÓN**',
        '',
        'Verifica tu cuenta para acceder a nuestra comunidad y descubrir todo lo que **RIXZY LEAKS** tiene para ofrecer.',
        '',
        '🇺🇸 **VERIFICATION**',
        '',
        'Verify your account to access our community and discover everything **RIXZY LEAKS** has to offer.',
        '',
        'Presiona el botón de abajo para verificarte.',
        '',
...
async function handleRemove(interaction, guild, client) {
    ...
    const result = await removeVerification(
        client,
        guild.id,
        targetUser.id,
        {
    ...
    if (result.status === 'not_verified') {
        return await InteractionHelper.safeReply(interaction, {
            embeds: [
                infoEmbed(
                    'Not Verified',
                    `${targetUser.tag} does not currently have the verified role.`
                )
            ],
